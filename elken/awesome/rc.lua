...
-- {{{ Autostart applications once at boot, ignore if already running
...
modkey     = "Mod4"
altkey     = "Mod1"
terminal   = "xterm -fg red -bg black"
...
-- Feel free to set your own programs too!
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({"I", "II", "III", "IV", "V", "VI"}, s, layouts[1])
end
-- }}}
...
-- RAM
ram_graph = blingbling.line_graph({
    height = 18,
    width = 100,
    show_text = true,
...
vicious.register(net, vicious.widgets.net, '<span color="#cc9393">Up: ${enp9s0 up_kb}</span> :: <span color="#7f9f7f">Down: ${enp9s0 down_kb}</span>',2)
